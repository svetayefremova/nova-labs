...
const ORGAN_ICONS: Record<string, IconName> = {
  heart: 'heart',
  lungs: 'lungs',
  liver: 'liver',
  kidneys: 'kidneys',
...
function getOrganIcon(sectionId?: string): IconName {
  if (!sectionId) return 'brain';
  return ORGAN_ICONS[sectionId.toLowerCase()] ?? 'brain';
}
...
export function HighRiskCard({ sectionId, sectionName, oncoRads }: Props) {
  ...
  const organIcon = getOrganIcon(sectionId);
    ...
    <View className="rounded-[20px] bg-white px-4 py-4 gap-4" style={shadows.card}>
      ...
      <View className="flex-row items-center gap-3">
        <View className="w-12 h-12 rounded-full bg-background items-center justify-center">
          <Icon name={organIcon} size={24} color={textColor} />
        </View>
      ...
      <View className="flex-row gap-3">
        ...
        <Button variant="tertiary" onPress={() => router.push('/(tabs)/(images)')}>
          <Button.Label>{t('profile.review_images')}</Button.Label>
        </Button>
      </View>
    </View>
  );
